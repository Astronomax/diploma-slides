...
#diagram-slide([Постраничный индекс в оперативной памяти], "images-transparent/page-index-0.svg")

#content-slide([Постраничный индекс в оперативной памяти])[
  #set text(size: 15pt)
...
#content-slide([Бенчмаркинг: методика и метрики])[
...
#multi-graph-slide(
  ...
  layout: "2x2",
  body: [
    Рост частоты попаданий в кэш согласуется с ростом пропускной способности: чем больше рабочий набор в кэшах,
...
#content-slide([Вывод])[
  ...
  Появляется дополнительный компонент амплификация чтения (read amplification) — чтение блоков с метаданными —
  который заметно влияет на пропускную способность при малых квотах кэшей.
]
