...
#content-slide([Бенчмаркинг: методика])[
  #set text(size: 14.5pt)
  #set par(leading: 0.5em)
  YCSB-подобные нагрузки Vinyl: workload A, B и C (`pico_ycsb`, набор ~200 МБ).

  Сравнивались две реализации постраничного индекса:
  - #strong[old] — метаданные страницы в RAM
  - #strong[new] — B-дерево на диске, в RAM только кэши (соотношение ключи : `vy_page_info` = 1:4)

  Квота кэшей варьировалась от 0 до ~2 МБ; резидентная верхушка B-дерева отключена (`d0`).
  Перед прогонами сбрасывались дисковые кэши ОС, `vinyl_cache = 0`.
]

#content-slide([Бенчмаркинг: метрики])[
  #set text(size: 14.5pt)
  #set par(leading: 0.5em)
  - #strong[throughput (OPS)] — операций в секунду
  - #strong[memory.page_index] — память структур постраничного индекса
  - #strong[read / write / space amplification]
  - для #strong[new]: hit rate и вытеснения по двум кэшам

  Варианты новой реализации: `q*_d0` — квота кэша `q*`, нулевая резидентная верхушка.
]
...
#graph-slide(
  ...
  body: [
    При нулевых квотах (`q0_d0`) throughput ниже, чем у `old`: A −33%, B −25%, C −58%.
    С ростом квоты новая реализация приближается к `old` и выходит на плато.
  ],
)
...
#content-slide([Просадка throughput при меньшей памяти])[
  #set text(size: 14pt)
  #table(
    columns: 4,
  ...
  В `new` память ограничена квотой кэшей; при малых квотах — дополнительный I/O.
  При росте квоты throughput растёт до плато.
]
...
#graph-slide(
  ...
  body: [
    Рост hit rate согласуется с ростом throughput: чем больше рабочий набор в кэшах,
